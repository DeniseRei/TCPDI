...
// -----------------------------------------------------------------------------
    ...
    protected function decodeXrefStream($startxref, $xref=array()) {
        ...
        foreach ($keys as $k => $key) {
            ...
            if (($key == '/Type') AND ($v[0] == PDF_TYPE_TOKEN) AND ($v[1] == 'XRef')) {
                ...
                // number of bytes (in the decoded stream) for the corresponding field
                $wb = array(
                    intval($v[1][0][1]),
                    intval($v[1][1][1]),
                    intval($v[1][2][1])
    ...
    private function _getPageRotation($obj) {
        ...
        if (is_array($obj) && isset($obj[1][1]['/Rotate'])) {
            $rotateValue = $obj[1][1]['/Rotate'];
            if (is_array($rotateValue)) {
                $res = $this->getObjectVal($rotateValue);
        ...
        if (is_array($obj) && !isset($obj[1][1]['/Parent'])) {
            return false;
        } else {
            $parentValue = $obj[1][1]['/Parent'];
            if (is_array($parentValue)) {
                $res = $this->_getPageRotation($parentValue);
                if (is_array($res) && $res[0] == PDF_TYPE_OBJECT) {
                    return $res[1];
                }
                return $res;
